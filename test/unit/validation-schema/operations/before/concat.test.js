...
test(`Doesn't mutate previous schema`, () => {
  const schema = {
    a: {
      a: Joi.any(),
      b: Joi.any()
...
test(`Throws when attempting to concat to a non validation`, () => {
  const toConcat = {
    a: Joi.any(),
    c: Joi.any()
  };
...
test(`Maintains in current and concats if exists`, () => {
  const exCurrent = () => ({
    a: {
      b: Joi.any(),
      c: Joi.any()
