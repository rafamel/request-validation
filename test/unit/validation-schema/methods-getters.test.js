...
test(`Method aliases are added`, () => {
  ...
  all.forEach((method) => {
    config.req.forEach((item) => {
      ...
      instance[methodName]();
      expect(ValidationSchema.prototype[method]).lastCalledWith(item);

      inputs.forEach((input) => {
        const calledWith = {};
        calledWith[item] = input.length > 1 ? deep(input) : deep(input[0]);

        instance[methodName](...input);
        expect(ValidationSchema.prototype[method]).lastCalledWith(calledWith);
      });
    });
  });
});

test(`Property aliases are added`, () => {
  const instance = new ValidationSchema();
  config.req.forEach((item) => {
