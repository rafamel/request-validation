...
describe(`No handler`, () => {
  ...
  describe(`On Fail`, () => {
    ...
    test(`Does not change req values (after fail)`, () => {
      const [t, data] = [exData(), exData()];
      validateRequest(t.toValidate, t.failOptions)(t.req, t.res, t.next);
    ...
    test(`Calls next with error .isRequestValidation = true`, () => {
      const t = exData();
      validateRequest(t.toValidate, t.failOptions)(t.req, t.res, t.next);

      expect(t.next).toHaveBeenCalledTimes(1);
      expect(t.next.mock.calls[0][0]).toBeInstanceOf(Error);
      expect(t.next.mock.calls[0][0]).toHaveProperty(
        'isRequestValidation',
        true
      );
    });
    test(`Calls next with error if exception happens`, () => {
      const t = exData();
      validateRequest(t.toValidate, t.errorOptions)(t.req, t.res, t.next);

      expect(t.next).toHaveBeenCalledTimes(1);
      expect(t.next.mock.calls[0][0]).toBeInstanceOf(Error);
      expect(t.next.mock.calls[0][0]).not.toHaveProperty('isRequestValidation');
    });
  });
});
...
describe(`Handler`, () => {
  describe(`On Pass`, () => {
    test(`Empty next, does not call handler`, () => {
      const t = exData();
      validateRequest(t.toValidate, t.options, t.handler)(t.req, t.res, t.next);
  ...
  describe(`On Fail`, () => {
    test(`Calls handler with error .isRequestValidation = true`, () => {
      const t = exData();
      ...
      expect(t.handler).toHaveBeenCalledWith(Error(), t.req, t.res, t.next);
      expect(t.handler.mock.calls[0][0]).toHaveProperty(
        'isRequestValidation',
        true
      );
    });
    test(`Calls handler with error if exception happens`, () => {
      const t = exData();
      ...
      expect(t.handler).toHaveBeenCalledTimes(1);
      expect(t.handler).toHaveBeenCalledWith(Error(), t.req, t.res, t.next);
      expect(t.handler.mock.calls[0][0]).not.toHaveProperty(
        'isRequestValidation'
      );
    });
  });
