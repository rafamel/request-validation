...
const { ValidationSchema } = require('../../src');
...
test(`schema.clear() works`, () => {
  const schemaA = exSchema('body')
    ...
    .presence('required');
  const schemaB = schemaA.clear().schema;
  ...
  config.req.forEach((item) => {
    test(`- ${item}: methods work + no mutation`, () => {
      ...
      const tests = [
        ...
        {
          schema: baseSchema.use(`${item}.a`).add(
            buildObj(item, {
        ...
        {
          schema: baseSchema.use(`${item}.a`).concat(
            buildObj(item, {
              a: Joi.string().min(6),
              i: Joi.number().required()
            })
        ...
        },
        {
          ...
          pass: [{ f: { b: 2, d: 2 } }, { f: { a: 'a', c: 'c', b: 4, d: 4 } }],
          fail: [
            { f: { d: 2 } },
            { f: { b: 2 } },
            { f: { a: 'a', c: 2, b: 4, d: 4 } },
      ...
      tests.forEach(({ schema, pass, fail }) => {
        if (pass) {
          pass.forEach((obj) => {
            expect(() => Joi.assert(obj, schema[item])).not.toThrow();
            expect(baseSchema).toEqual(exSchema(item));
          });
        }
        if (fail) {
          fail.forEach((obj) => {
            expect(() => Joi.assert(obj, schema[item])).toThrow();
          });
        }
      });
    });
...
describe(`Schema build: options and empty keys cleanup`, () => {
  test(`Options fails on non existent keys`, () => {
    const schema = new ValidationSchema({
      body: { a: Joi.any() }
    });
    ...
    expect(() => schema.presence({ body: 'required' }).schema).not.toThrow();
    expect(
      () => schema.options({ headers: { presence: 'required' } }).schema
    ).toThrow();
    expect(() => schema.presence({ headers: 'required' }).schema).toThrow();
  });
  test(`Empty keys are cleaned`, () => {
    const schema = new ValidationSchema({
      body: { a: Joi.any() },
      headers: {
        a: Joi.any(),
