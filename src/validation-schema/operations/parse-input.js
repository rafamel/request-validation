...
const keyPathError = Error(
  `Failed to validate standard key path arguments. Please check the ${config.moduleName} documentation.`
);
  ...
  function helper(input, prefix = '') {
    function parseStr(str) {
      ...
      (prefix + str).split('.').forEach((x, i, arr) => {
        if (Object.hasOwnProperty.call(current, x) && i < arr.length - 1) {
          current = current[x];
          return;
        }
        current[x] = {};
...
function stringPaths(input) {
  ...
  arr.forEach((item) => {
    ...
    if (!child) return;
    if (!Object.hasOwnProperty.call(ans, parent)) {
      ans[parent] = [];
    }
